...
class BookService with ChangeNotifier {
  ...
  Future<app_models.Book?> getBookDetailsById(int bookId) async {
    ...
    final List<Map<String, dynamic>> bookMaps = await db.query('Books',
        where: 'b_id = ?', whereArgs: [bookId], limit: 1);
  ...
  Future<void> loadLibraryBooks() async {
    ...
    final List<Map<String, dynamic>> bookMaps = await db.rawQuery(
        'SELECT B.* FROM Books B INNER JOIN Library L ON B.b_id = L.b_id WHERE L.u_id = ? ORDER BY L.l_id DESC',
        [_currentUserId]);
    if (bookMaps.isEmpty) {
      _libraryBooks = [];
      _isLoading = false;
  ...
  Future<List<app_models.Note>> getNotesForBook(int bookId) async {
    ...
    final noteMaps = await db.query('Notes',
        where: 'b_id = ?', whereArgs: [bookId], orderBy: 'n_id DESC');
    return noteMaps
  ...
  Future<app_models.Book?> findBookInLibraryByWorkId(String workId) async {
    ...
    final List<Map<String, dynamic>> results = await db.rawQuery(
        'SELECT B.* FROM Books B INNER JOIN Library L ON B.b_id = L.b_id WHERE L.u_id = ? AND B.b_oWorkId = ?',
  ...
  Future<int> _getOrInsert(DatabaseExecutor txn, String table, String column,
    ...
    final results = await txn.query(table,
        ...
        whereArgs: [value],
        limit: 1);
    if (results.isNotEmpty) return results.first[idColumn] as int;
