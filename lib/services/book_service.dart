...
class BookService with ChangeNotifier {
  ...
  Future<void> loadLibraryBooks() async {
    ...
    final authorRelationMaps = await db.rawQuery('SELECT BA.b_id, A.a_id, A.a_name FROM Author A INNER JOIN Book_Author BA ON A.a_id = BA.a_id WHERE BA.b_id IN (${bookIds.map((_) => '?').join(',')})', bookIds);
    
    final Map<int, List<app_models.Author>> authorsByBookId = {};
    for (var map in authorRelationMaps) {
      final bookId = map['b_id'] as int;
      final author = app_models.Author(id: map['a_id'] as int, name: map['a_name'] as String);
  ...
  Future<bool> addBookFromApi(ApiBookSearchResult apiBook) async {
    ...
    if (existingBooks.isNotEmpty) {
      ...
      bookId = await db.transaction((txn) async {
        final newBookId = await txn.insert('Books', {'b_name': apiBook.title, 'b_coverUrl': apiBook.coverId != null ? 'https://covers.openlibrary.org/b/id/${apiBook.coverId}-M.jpg' : null, 'b_description': details?.description, 'b_oWorkId': apiBook.workKey, 'b_totalPages': details?.totalPages, 'b_publishDate': details?.publishDate});
        for (String name in apiBook.authors) { int id = await _getOrInsert(txn, 'Author', 'a_name', 'a_id', name); await txn.insert('Book_Author', {'b_id': newBookId, 'a_id': id}); }
        if (details != null) {
          for (String name in details.publishers) { int id = await _getOrInsert(txn, 'Publisher', 'pbl_name', 'pbl_id', name); await txn.insert('Book_Publisher', {'b_id': newBookId, 'pbl_id': id}); }
          for (String name in details.subjects) { int id = await _getOrInsert(txn, 'Subject', 'sbj_name', 'sbj_id', name); await txn.insert('Book_Subject', {'b_id': newBookId, 'sbj_id': id}); }
          for (String name in details.people) { int id = await _getOrInsert(txn, 'Person', 'prs_name', 'prs_id', name); await txn.insert('Book_Person', {'b_id': newBookId, 'prs_id': id}); }
          for (String name in details.places) { int id = await _getOrInsert(txn, 'Place', 'plc_name', 'plc_id', name); await txn.insert('Book_Place', {'b_id': newBookId, 'plc_id': id}); }
          for (String name in details.times) { int id = await _getOrInsert(txn, 'Time', 't_name', 't_id', name); await txn.insert('Book_Time', {'b_id': newBookId, 't_id': id}); }
        }
        return newBookId;
      });
    }
    final analyticsEntry = await db.query('Analytics', where: 'b_id = ? AND u_id = ?', whereArgs: [bookId, _currentUserId]);
    if (analyticsEntry.isEmpty) {
      final newAnalytics = app_models.Analytics(userId: _currentUserId, bookId: bookId, status: 'wishlist', currentPage: 0, lastReadAt: DateTime.now());
  ...
  Future<app_models.Book?> getBookDetailsById(int bookId) async {
    ...
    return app_models.Book(id: bookId, name: bookMap['b_name'] as String?, coverUrl: bookMap['b_coverUrl'] as String?, description: bookMap['b_description'] as String?, oWorkId: bookMap['b_oWorkId'] as String?, totalPages: bookMap['b_totalPages'] as int?, publishDate: bookMap['b_publishDate'] as String?, authors: authors, publishers: publishers, subjects: subjects, people: people, places: places, times: times, analytics: app_models.Analytics.fromMap(bookMap));
  }
  ...
  Future<app_models.Book?> findBookInLibraryByWorkId(String workId) async {
    final db = await _db.database;
    final List<Map<String, dynamic>> results = await db.rawQuery('SELECT B.* FROM Books B JOIN Analytics A ON B.b_id = A.b_id WHERE A.u_id = ? AND B.b_oWorkId = ?', [_currentUserId, workId]);
    if (results.isEmpty) return null;
    final bookMap = results.first;
    return app_models.Book(id: bookMap['b_id'] as int, name: bookMap['b_name'] as String?, oWorkId: bookMap['b_oWorkId'] as String?);
  }
  ...
  Future<StatsData> getStats() async {
    ...
    final completedBooks = allAnalytics.where((m) => m['a_status'] == 'completed');
    ...
    if (booksReadCount > 0) {
      ...
      totalPagesRead = pageCounts.fold(0, (sum, map) => sum + (map['b_totalPages'] as int? ?? 0));
    }
    final now = DateTime.now();
    ...
    int booksToday = completedBooks.where((m) => m['a_finishedAt'] != null && DateTime.parse(m['a_finishedAt'] as String).isAfter(today)).length;
    int booksWeek = completedBooks.where((m) => m['a_finishedAt'] != null && DateTime.parse(m['a_finishedAt'] as String).isAfter(startOfWeek)).length;
    int booksMonth = completedBooks.where((m) => m['a_finishedAt'] != null && DateTime.parse(m['a_finishedAt'] as String).isAfter(startOfMonth)).length;
    int booksYear = completedBooks.where((m) => m['a_finishedAt'] != null && DateTime.parse(m['a_finishedAt'] as String).isAfter(startOfYear)).length;
    return StatsData(totalBooks: allAnalytics.length, booksRead: booksReadCount, pagesRead: totalPagesRead, booksReadByPeriod: {'daily': booksToday, 'weekly': booksWeek, 'monthly': booksMonth, 'yearly': booksYear});
  }
  ...
  Future<void> addNoteForBook(String text, int bookId) async {
    final db = await _db.database;
    await db.insert('Notes', {'u_id': _currentUserId, 'b_id': bookId, 'n_text': text, 'n_createdAt': DateTime.now().toIso8601String()});
    notifyDataChanged();
  }
  ...
  Future<List<app_models.Note>> getNotesForBook(int bookId) async {
    final db = await _db.database;
    final noteMaps = await db.query('Notes', where: 'b_id = ? AND u_id = ?', whereArgs: [bookId, _currentUserId], orderBy: 'n_createdAt DESC');
    return noteMaps.map((m) => app_models.Note(id: m['n_id'] as int, bookId: m['b_id'] as int, text: m['n_text'] as String, bookTitle: '')).toList();
  }
